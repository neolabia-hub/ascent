...
ASISTENCIA (evidencia de auditoría), tres mecanismos combinables por convocatoria:
  ...
  VA CON EL `kind`, NO CON LA MODALIDAD (2026-09-05, corrigiendo lo que este documento decía).
  Aquí ponía "asistencia PRESENCIAL", y atarlo a la modalidad deja fuera el caso que más ha
  crecido: el webinar en vivo de la ARL. Una jornada `EVENT` se cierra por lista la dicte como la
  dicte —hay quien estuvo, y no queda contenido completado en la plataforma—; una `PERMANENT` no,
  porque ahí la persona entra sola y la evidencia es justamente lo que el sistema registró.
