...
ASISTENCIA (evidencia de auditoría), tres mecanismos combinables por convocatoria:
  ...
  2. Código QR de sesión (rotativo, con vencimiento): el asistente lo escanea desde su celular;
     queda usuario autenticado + sello de tiempo exacto. CONSTRUIDO el 2026-09-08
     (`asistencia-en-sala.service.ts`, `method: QR`). **Rota cada 90 segundos** — un código fijo se
     fotografía y se manda al grupo, y quien está en su casa marca asistencia a una jornada a la que
     no fue— y se puede **dictar en voz alta**: seis caracteres sin parecidos (nada de 0/O ni 1/I/L),
     porque siempre hay alguien con la cámara rota y ese es justo el que se queda sin constar.
  3. Firma en pantalla del asistente: se captura y almacena como imagen; el sistema genera el
     ACTA de la sesión (PDF) con la lista, firmas y metadatos. CONSTRUIDO el 2026-09-08
     (`acta-de-sesion.service.ts`, `method: SIGNATURE`). La **huella del acta** es SHA-256 de lo que
     el acta AFIRMA —quién, cuándo, quién asistió con qué estado y firma— y no de los bytes del PDF,
     que cambian con la fecha de generación: así dos copias del mismo contenido tienen la misma
     huella. Volver a generarla NO pisa la anterior, porque un acta es un documento con fecha.
     Con ellos llegó `attendance:sign`, que es de TODO usuario autenticado y solo sobre lo SUYO.
...
| 160 | **La VIA C —quien llega ya certificado de otro empleo— vive en la OBLIGACION, no en una inscripcion. Y CONVALIDAR NO ES EXIMIR.** Migracion `20260908120000`: cinco columnas en `assignments` (`ext_cert_issuer`, `ext_cert_number`, `ext_cert_file_key`, `convalidated_by`, `convalidated_reason`), reutilizando `valid_until_override` para la vigencia | **Donde vive.** El papel de un tercero vivia en `enrollments`, y `enrollments.offering_id` es OBLIGATORIO: quien nunca fue a una jornada no tiene inscripcion donde colgarlo. Se evaluo `certification_grants` —que parecia el sitio— y exige `source_enrollment_id`: el mismo problema. La OBLIGACION es la unica fila que existe sin jornada, y ademas es la que el auditor rastrea y la que el motor lee para saber cuando vuelve a deberse. **Por que columnas propias y no `waived_reason`.** Porque son dos hechos distintos: `WAIVED` le dice al auditor *"la dejamos pasar"* y convalidar dice *"la hizo, en otro sitio, y aqui esta el papel"*. La primera es una excepcion y la segunda es CUMPLIMIENTO. Meterlas en la misma columna obligaria a leer un texto libre para distinguirlas, y ningun informe puede hacer eso. Por eso la obligacion convalidada queda `COMPLETED`, sin `completed_enrollment_id` —enlazar una ejecucion inventada falsearia el rastro—. **La vigencia no se duplica:** `valid_until_override` ya significaba "hasta cuando vale de verdad cuando lo dice un papel" (#157), y un papel de otro empleo es exactamente eso, asi que el motor no aprende ningun caso nuevo. Probado en `convalidar-papel-ajeno.mjs` |
| 161 | **Aceptar una certificacion previa se decide en DOS capas: la formacion declara si su papel es transferible (por defecto NO), y aceptar cada papel concreto es un acto con nombre, fecha y motivo.** Bandera `admiteConvalidacion` con cascada tipo -> ficha (migracion `20260908160000`) | Lo pregunto el cliente y su intuicion fijo el diseño: *"¿y si la empresa cree que debe hacerlo igual porque son procesos propios? ¿no seria mejor que no le valga el certificado y lo obligue a tomarlo?"*. **Su caso no es la excepcion, es la mayoria:** una induccion no la exime ningun papel de otra empresa —por definicion enseña los procedimientos de ESTA— y hay empresas que aun con una recertificacion legal exigen su propia sesion. Por eso el defecto es NO. **Y por eso es configuracion y no un motivo libre por persona:** si dependiera de quien registra, un descuido daria por cumplida una induccion con un papel ajeno. Lo que SI queda a criterio es aceptar un papel CONCRETO —¿organismo acreditado? ¿vigente? ¿mismo alcance?—, y eso ningun sistema lo lee de un PDF. **Las dos capas hacen falta:** sin la primera se convalida lo que no se debe; sin la segunda, se acepta cualquier papel. La cascada tipo -> ficha se añadio el mismo dia y no dos despues, porque a `tracksExternalCertificate` le habia pasado exactamente eso 48 horas antes: nacio por tipo, aparecio el caso que no encajaba (dentro de "Recertificacion" conviven una de alturas, transferible por norma, y una interna sobre un equipo propio, que no lo es) y hubo que migrar. Es la cuarta bandera que sigue el patron de la #111 |
| 162 | **El informe de Vencimientos se parte por el TRABAJO que genera cada fila —REPROGRAMAR o PERSEGUIR—, no por la tabla de la que salio el dato. Y cada persona y formacion aparece UNA vez** | Nacio partiendo por el origen: «Certificacion» leia `certification_grants` y «Obligacion» leia `assignments`. Eso es una division del esquema, no del trabajo, y costaba dos cosas. **(a)** `certification_grants` no la escribe ningun modulo, asi que esa serie salia siempre en CERO mientras las fechas de caducidad existian de verdad en `assignments.valid_until_override` (#157) y `certificates.valid_until` (#111): la serie no estaba vacia por falta de dato, estaba leyendo la tabla equivocada. **(b)** Y a quien esta en su ventana de 60 dias —el papel le caduca en marzo y la ronda siguiente ya le nacio— se le contaba en las DOS series: el mismo trabajo, dos veces, y justo en lo que esta a punto de pasar, que es la unica parte que alguien mira. El eje bueno es el que decide que se hace: reprogramar ocupa un salon y un dia del año que viene; perseguir es llamar a alguien esta semana. Al consolidar manda la **obligacion abierta** —es la que tiene plazo de verdad y la que se cierra al tomar la lista— y, sin ella, la caducidad mas proxima. Cada fila dice ademas **segun que** vence, porque quien lee "vence en marzo" pregunta siempre "¿segun que?". Recorrido: `vencimientos.mjs` |
| 163 | **Una obligacion VIVA por persona y formacion, venga de la regla que venga. Y un criterio de pantalla compartido por dos puertas vive en un solo sitio y se importa** | Dos caras del mismo principio —una sola implementacion de una sola regla— y las dos se pagaron el mismo dia. **La obligacion:** una formacion exigida por dos reglas vivas le nacia dos veces a la misma persona; la asignacion manual ya se protegia de eso (*"no pisa lo que ya esta vivo"*) y el motor no. Se creia raro —«solo en alcance MANUAL, no se ha visto en la practica»— y era comun: publicar una induccion **crea sola** su regla de "toda la empresa", asi que cualquier induccion publicada y ademas exigida a un cargo ya tenia dos. Lo que se veia no era un error visible sino un denominador inflado: el cumplimiento bajaba sin que nadie dejara de hacer nada. **El criterio:** «si la dicta la empresa, no hay tercero que certifique» vivia dentro de la lista de asistencia, y la segunda puerta —*Papeles de un tercero*, en la ficha— no se entero: enseñaba filas que decian *"la dicto PROPIOS"* al lado del campo para el numero del certificado externo. Ahora vive en `certificate-policy.ts` y las dos lo importan. Un criterio copiado se separa; uno importado, no. Recorridos: `dos-reglas-una-obligacion.mjs` y `dos-puertas-del-papel.mjs` |
...
| 171 | **Un PROGRAMA agrupa formaciones y certifica por el conjunto, y para eso NO se construyó un motor nuevo**: se reusó el esquema `LearningPath`/`PathItem`/`PathEnrollment`, que llevaba desde el Sprint 0 en Prisma sin usar, y asignar un programa **delega en el motor de siempre** — una llamada a `setActivityRequirement` por módulo, misma audiencia. Un programa **no tiene tipo** (`ActivityType`) ni recurrencia propia | El cliente lo pidió con un caso concreto: *«Gestión Humana», «Comercial»… se certifican como un solo «Programa de Inducción General», con reglas por tenant («7 de 9, con alguno obligatorio sí o sí»)*. La tentación era un brazo `PATH` paralelo en el motor de obligaciones, y habría duplicado **todo** lo que ya está probado: plazos, rondas, retiradas, exenciones, el informe. Delegando, un programa no es un mecanismo nuevo sino una **vista sobre obligaciones normales**, y por eso funciona sin tocarlo nada de lo que ya existía — incluida la regla PLAN. Lo mismo con el tipo: el tipo decide si algo certifica, cada cuánto se repite y cuándo vence, y todo eso ya vive en los MÓDULOS. Dárselo también al programa sería la misma verdad en dos sitios, y el día que discrepen no hay forma de saber cuál manda. Un programa certifica siempre al completarse, sin casilla que lo decida. Y si es módulo de un programa PUBLICADO, la formación **deja de emitir constancia individual**: la evidencia que vale es la del conjunto |
| 172 | **NADA PUEDE QUEDAR SIN HACER: un cupo perdona lo REPROBADO, nunca lo no intentado.** `evaluarPrograma` exige tres cosas —obligatorios aprobados, cupos cubiertos y **cero módulos con obligación viva** (`PENDING`, `IN_PROGRESS`, `OVERDUE`)—. Para resolver lo que no se hizo están **eximir** (resuelve, NO aprueba) y **convalidar** (cierra como cumplido) | Lo dijo el cliente en plano: *«no se puede aceptar nada que esté sin hacer; si no hace 1 no puede aprobar aunque diga 6 de 7»*. Con solo las dos primeras condiciones, un programa de «5 de 7» se completaba **ignorando dos módulos**: la persona no se presentaba, el cupo daba, y el sistema emitía una constancia que afirma una formación que nadie cursó. Es el único fallo de todo el módulo que producía **evidencia falsa**, que es la clase que no se negocia. La diferencia con el cupo es de intención: reprobar es haberse presentado y no alcanzar —eso el cupo lo puede absorber—; no presentarse no es un resultado, es una tarea abierta. Y cuando hay un motivo real (incapacidad, ya lo traía de otro empleo) existe un acto **con nombre, fecha y motivo escrito**, que es justo lo que el auditor necesita leer y lo que un cupo silencioso destruía |
| 173 | **Un programa publicado solo lo ve QUIEN ESTÁ OBLIGADO**, no toda la plantilla: `misProgramas` cruza los publicados contra las obligaciones vivas de sus módulos y las inscripciones existentes, y las retiradas (`WITHDRAWN*`) no cuentan | Publicar era **la única** condición para que un programa apareciera en «Mi aprendizaje» de todo el mundo. Un Programa de Inducción General dirigido a quien ingresa le salía al conductor con quince años en la empresa, en su lista de cosas por hacer. El cliente lo cortó en seco: *«no puede pasar que un programa quede visible a todos; solo los obligados deben verlo»*. Y tiene un efecto peor que el ruido: una lista que mezcla lo que me toca con lo que no **deja de ser una lista de lo que me toca**, y entonces no se mira. La visibilidad se deriva de la obligación en vez de guardarse aparte por lo de siempre: un segundo sitio donde apuntar quién ve qué es un segundo sitio que se queda viejo |
| 174 | **La cobertura de un programa se lee POR MÓDULO —a quién alcanza cada uno— y no como un agregado con la cifra entre paréntesis.** Y **no se bloquea** agregar un módulo sin publicar ni sin obligaciones: se avisa | La ficha decía a quién se le exigía colgando de cada audiencia un **«(solo 2 de 5)»**: avisa de que faltan tres y **no dice cuáles**, así que había que abrir formaciones hasta dar con las que fallan. El cliente lo pidió del derecho —*«lo importante es saber por módulo: inducción general cubre toda la empresa o lo que seleccionaron»*— y la pregunta que se hace de verdad no es «¿a cuántos módulos llega esta audiencia?» sino, mirando una fila, **«¿y este, a quién?»**. Las dos vistas salen de **una sola lectura** de las reglas, invertida de dos maneras, así que no pueden discrepar. Los bloqueos se descartaron porque **invierten el orden natural del trabajo**: un programa se arma *antes* de decidir a quién se le exige —el botón de asignar existe justo para exigir los cinco módulos de una vez— y mientras sus formaciones aún se escriben. Se bloquea lo que produce evidencia falsa (#172); lo que solo está a medias, se avisa |
